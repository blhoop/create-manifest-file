...
| 1.8.0 | 2026-03-27 | `spoke_id` in `ctm_properties` for globally-unique resource naming. Optional 2-5 char alphanumeric discriminator fused with product in names (e.g. `lb` + `03` → `lb03`). All 62 modules updated. Backward compatible — omitting spoke_id produces current naming. |
